...
<br>
 ...
 <center>
    ...
    try{
      $sql1="SELECT * FROM pescado WHERE lote=".$_REQUEST['cod'].";";
      $resultado1=$base->prepare($sql1);
      $resultado1->execute(array());
      while ($consulta1=$resultado1->fetch(PDO::FETCH_ASSOC)) { 
    ?>

     <form method="POST" name="form" id="formu" action="act_pescado.php?cod=<?php echo $_REQUEST["cod"]; ?>">
        <table class="tabcen" align="center">

          <tr>
              <td colspan="2" align="center"><h2><strong>FORMULARIO ACTUALIZACIÓN DE PESCADO</strong></h2></td>
            </tr>
                 <tr>
          <td class="negrita">
          ...
          <td>
          <input type="number" name="pes" value="<?php echo $consulta1['pesopromedio'];?>" required="">
        </td>
        </tr>
        <tr>
          <td class="negrita">
          ...
          <td>
          <input type="number" name="canti" value="<?php echo $consulta1['cantidad'];?>" required="">
        </td>
        </tr>
        <tr>
        <td class="negrita">
        ...
        <td>
          <select name="select" required="">
            <option value="<?php echo $consulta1['fk_tipo'];?>">Seleccionar especie...</option>
              <?php
              try{
                $sql1="SELECT *FROM  tipopescado";
                $result=$base->prepare($sql1);
                $result->execute(array());
                while ($consulta=$result->fetch(PDO::FETCH_ASSOC)) {
                  ?>
                  <option value="<?php echo $consulta['idtipo']; ?>"><?php echo $consulta['especie']; ?></option>
              <?php
                }
              }
              catch(Exeption $e){
        ...
        <tr>
          ...
          <td>
          <input type="number" name="valorc" value="<?php echo $consulta1['valorcompra'];?>" required="">
        </td>
        </tr>
        <tr>
          <td class="negrita">
          ...
          <td>
          <input type="number" name="valorv" value="<?php echo $consulta1['valorventa'];?>" required=""> 
        </td>
        </tr>
        <tr>
          <td class="negrita">
          ...
          <td>
          <input type="date" name="fecha" value="<?php echo $consulta1['fechaingreso'];?>" required="">
        </td>
        </tr>
        <tr>
          <td colspan="2" align="center"><input id="boton" type="submit" name="btn" value="Actualizar"></td>
        </tr>
      </table>
    </form>
    </center>
   ...
   if(isset($_POST['btn'])){
    ...
    $especie=$_POST['select'];
    $valcomp=$_POST['valorc'];
    $valven=$_POST['valorv'];
    $fech=$_POST['fecha'];

    try{
      $sql="UPDATE pescado SET `pesopromedio`=:pe,`cantidad`=:cant,`fk_tipo`=:esp,`valorcompra`=:valc,`valorventa`=:valv,`fechaingreso`=:fec WHERE lote=".$_REQUEST["cod"].";";
      $resultado=$base->prepare($sql);//Con esto se prepara la ejecución de mi sentencia
      $resultado->execute(array(":pe"=>$peso, ":cant"=>$cantidad, ":esp"=>$especie, ":valc"=>$valcomp, ":valv"=>$valven, ":fec"=>$fech)); //Ejecución de la sentencia SQL
    ?>
    <script language="javascript"> window.alert("Actualización de pescado exitoso !!"); 
      window.location="conpescado.php"</script>
